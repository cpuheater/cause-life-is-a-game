...
class sil_module:
    ...
    def step(self, obs, actions, rewards, dones, invalid_action_masks):
        for n in range(self.args.num_envs):
            self.running_episodes[n].append([obs[n], actions[n], rewards[n], invalid_action_masks[n]])
    ...
    def train_sil_model(self):
        for n in range(self.args.num_update):
            ...
            if obs is not None:
                ...
                invalid_action_masks = torch.tensor(invalid_action_masks, dtype=torch.float32).unsqueeze(1)
                ...
                # start to next...
                _, action_log_probs, dist_entropy, _ = self.network.get_action(obs, actions.reshape(actions.shape[0], -1).T,
                                                                                        invalid_action_masks.reshape(invalid_action_masks.shape[0], -1), self.envs)
    ...
    def update_buffer(self, trajectory):
        ...
        for (ob, a, r, invalid_action_mask) in trajectory:
            if r > 0:
    ...
    def add_episode(self, trajectory):
        ...
        for (ob, action, reward, invalid_action_mask) in trajectory:
            if ob is not None:
                obs.append(ob)
            else:
                obs.append(None)
