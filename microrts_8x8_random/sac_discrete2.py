...
if __name__ == "__main__":
    ...
    parser.add_argument('--gym-id', type=str, default="Microrts8-randomBiasedAI",
...
class Policy(nn.Module):
    def __init__(self, mapsize=8*8):
        ...
        self.logits = layer_init(nn.Linear(128, mapsize * env.action_space.nvec[1:].sum()), std=0.01)
...
for global_step in range(1, args.total_timesteps+1):
    ...
    try:
        ...
        e.printStackTrace()
        raise
    rb.put((obs.squeeze(), action.cpu().numpy().squeeze(), reward, next_obs.squeeze(), done, invalid_action_mask.cpu().numpy()))
