...
def create_env() -> ViZDoomEnv:
    def thunk():
        ...
        env = gymnasium.wrappers.RecordEpisodeStatistics(ViZDoomEnv(game, channels=args.channels))
        return env 
    return thunk
...
envs = gymnasium.vector.SyncVectorEnv(
        [create_env() for i in range(args.num_envs)],
    )
assert isinstance(envs.single_action_space, gymnasium.spaces.discrete.Discrete), "only discrete action space is supported"
...
for update in range(1, num_updates+1):
    ...
    for step in range(0, args.num_steps):
        ...
        next_obs, rs, terminations, truncations, infos = envs.step(action)
        next_done = np.logical_or(terminations, truncations)
        next_obs = torch.Tensor(next_obs).to(device)
        rewards[step], next_done =torch.tensor(rs).to(device).view(-1), torch.Tensor(next_done).to(device)
