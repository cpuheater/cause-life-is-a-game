...
class ImpalaCNNLarge(nn.Module):
    ...
    """
    def __init__(self, in_depth, actions, linear_layer, model_size=1, spectral_norm='all'):
        super().__init__()
        ...
        self.critic = nn.Sequential(linear_layer(2048*model_size, 256),
                          nn.ReLU(),
                          linear_layer(256, 1))
        
        self.actor = nn.Sequential(linear_layer(2048*model_size, 256),
                          nn.ReLU(),
...
class Agent(nn.Module):
    def __init__(self, envs, frames=1):
        ...
        self.network =  ImpalaCNNLarge(frames, envs.action_space.n, nn.Linear)
