...
if __name__ == "__main__":
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            ...
            entropies = np.full(1, np.log(np.prod(envs.action_space.n)))
        else:
            actions, _, _, entropies = actor.get_action(torch.Tensor(obs).to(device).unsqueeze(0))
        ...
        rb.add(obs, real_next_obs, actions, rewards, entropies, done, info)
        ...
        if global_step > args.learning_starts:
            if global_step % args.update_frequency == 0:
                ...
                # CRITIC training
                with torch.no_grad():
                    _, next_state_log_pi, next_state_action_probs, _ = actor.get_action(data.next_observations)
                    ...
                    min_qf_next_target = next_state_action_probs * (
                        qf_estimates - alpha * next_state_log_pi
                    )
                ...
                if args.double_avg_w_q_clip:
                    # Q-clipping (SD-SAC)
                    with torch.no_grad():
                        qf1_old_target = qf1_target(data.next_observations)
                        qf2_old_target = qf2_target(data.next_observations)
                ...
                # ACTOR training
                _, log_pi, action_probs, entropy_pi = actor.get_action(data.observations)
                with torch.no_grad():
                    qf1_values = qf1(data.observations)
                    qf2_values = qf2(data.observations)
                    ...
                    if args.double_avg_w_q_clip:
                    # Double average Q-Value estimation (SD-SAC)
                        min_qf_values = torch.mean(torch.stack([qf1_values, qf2_values], dim=0), dim=0)
                    else:
